...
# --- kinburg-nodes: `audiosr.clap` was removed from this vendored copy (nothing in the
# super-resolution path built it — see the note in latent_diffusion/models/ddpm.py). The class below
# is upstream's, untouched apart from taking its two CLAP imports at construction time instead of at
# module import: the file still imports, and building this conditioner raises a plain ImportError
# saying which package is absent rather than breaking every import in the package.
def _clap_imports():
    from audiosr.clap.open_clip import create_model
    from audiosr.clap.training.data import get_audio_features
    return create_model, get_audio_features
...
class CLAPAudioEmbeddingClassifierFreev2(nn.Module):
    def __init__(
        ...
        create_model, get_audio_features = _clap_imports()
    ...
    def forward(self, batch):
        ...
        if self.embed_mode == "audio":
            ...
            with torch.no_grad():
                ...
                _, get_audio_features = _clap_imports()
