...
class DDPM(nn.Module):
    ...
    def __init__(
        ...
        # --- kinburg-nodes: `self.clap` was built here and never read anywhere in this package —
        # a leftover of the AudioLDM lineage this model grew from. Super-resolution conditions on
        # `VAEFeatureExtract` (see the shipped config), not on CLAP, so the module cost 0.80 GB of
        # the checkpoint's weights and 3.3 MB of source to sit unused. Both are gone; the
        # `clap.model.*` tensors in a checkpoint now land in `load_state_dict`'s "unexpected" list,
