...
# --- kinburg-nodes: set to a callable(step, total) to take over progress reporting and
# interruption; see the note in `ddim_sampling` below. None = upstream behaviour (its own tqdm).
STEP_HOOK = None
...
class DDIMSampler(object):
    ...
    def ddim_sampling(
        ...
        # --- kinburg-nodes: one hook so the host can drive its own progress bar and cancel a chunk
        # part-way through. Left None by default, so this file behaves exactly as upstream when the
        # vendored package is used on its own. See audio_sr/nodes.py.
        iterator = time_range if STEP_HOOK is not None else tqdm(
            time_range, desc="DDIM Sampler", total=total_steps
        )

        for i, step in enumerate(iterator):
            if STEP_HOOK is not None:
                STEP_HOOK(i, total_steps)
