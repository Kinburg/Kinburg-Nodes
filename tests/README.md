...
| `test_llm_server_options.py` | the server nodes' settings against the two command lines they have to become — every flag read out of the real binaries' own `--help`. The neutral rule (a widget left alone emits nothing), the polarity traps (llama.cpp's Flash Attention is off until asked, koboldcpp's is on until refused), the dialect (`-ncmoe` vs `--moecpu`, `-ctk/-ctv` vs `--quantkv`), what koboldcpp has no word for and the node therefore reports, and the embedding split — a second command line on llama-server, inline flags on koboldcpp |
| `test_llm_gateway.py` | the **LLM gateway** end to end against a stand-in backend that is a real subprocess (same argv shape, same `/health` probe, real sockets): connecting with nothing loaded, the first POST loading the model and being forwarded, SillyTavern's `dry_sequence_breakers` string reaching the model as an array, an unknown field named in a 400 being dropped and retried, streaming passing through chunk by chunk, and every way the VRAM goes back — the explicit unload, the queue hook that spares prompts holding the pack's own two nodes, and the reload on the next request |
...
| `test_echo_translit.py` | Echo's romanizer — that nothing outside `MMS_FA`'s 28-character dictionary can ever reach the model, the uk/ru fork on `г` and `и` guessed per word from the letters only one language has, the silent soft signs against a real inner apostrophe, and that the DISPLAY spelling survives everything the aligner needed |
| `test_echo_subs.py` | Echo's subtitle files — the three parts of ASS that are byte order rather than taste: `&HAABBGGRR` with blue first and alpha meaning transparency, Primary/Secondary being the sung/unsung pair the names suggest backwards, and 400 random lines proving the `\k` runs sum to the event's own length instead of drifting a centisecond per word |
| `test_echo_track.py` | Echo's windowing — a plan row is the alignment unit (so sub-sections alternating voices stay together), a window is bounded by its section and never by the song, the corrected span is where the words are rather than where the plan said, and an unspellable word is carried onto its neighbours instead of vanishing off the screen |
| `test_echo_align.py` | The aligner's orchestration over a STUB that obeys the one real constraint (words must be placed INSIDE the window given) — anchoring reproduces the author's own take: un-anchored 1 of 6 sections fits its window, anchored 6 of 6, and the never-sung Outro stops being laid on top of the chorus that IS singing. Plus the parts that need no weights — the encode-window split that must never lose a line (with overlapping seams so a word on a split point is still inside some window), and the length-weighted word confidence |
| `test_echo_plan.py` | The corrected plan Echo writes back, built from the author first real song — a section found INSIDE the one before it used to make a negative-length row that clamped to `0.00s` and was then dropped by the parser, deleting a section from the table whose job is to be trustworthy. Now emitted in found order, every row positive, the overlap named. Also that five instrumental `Break` rows against one marker is not a fault worth warning about, while a chorus sung more often than the plan allows still is |
| `test_echo_burn.py` | Karaoke painted onto a frame — that a line is laid out and drawn ONCE for the whole render (asserted by counting layouts after 240 frames, not by trusting the comment), that the wipe only ever advances, that a line lands inside the frame in all three positions and wraps when it is too wide, and that the singer colour reaches the pixels only after the words are sung. PIL only |
