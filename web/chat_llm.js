...
//   • Personas: one chip per connected persona_1..4 input, each a whole Local LLM Settings bundle
...
const VIEW_MIN = 96, INPUT_H = 56, GAP = 6, BAR_MIN = 24, STAT_MIN = 14;
...
function injectStyle() {
  ...
  .kb-lc-input{flex:0 0 auto;height:56px;resize:none;box-sizing:border-box;background:#181818;color:#e6e6e6;border:1px solid #2b2b2b;border-radius:6px;padding:6px 8px;font:12px/1.45 -apple-system,Segoe UI,sans-serif;outline:none;}
  `;
...
function render(node) {
  ...
  if (busy(node)) {
    if (node._kbPending != null) el.appendChild(makeBubble(node, { role: "user", content: node._kbPending }, -1));
...
function measureRows(node) {
  ...
  if (b !== node._kbBarH || s !== node._kbStatH) {
    node._kbBarH = b; node._kbStatH = s;
...
function rerunAt(node, i) {
  ...
  const h = getHistory(node);
  const t = turnAt(h, i);
  if (!t || (t.mode === "turn" && !t.text.trim())) return;
...
function doSend(node, opts) {
  ...
  if (plan.mode === "turn" && !plan.text) { renderPersonaBar(node); render(node); return; }
...
function setup(node) {
  ...
  wrap.append(box, stat, bar, input);
  node._kbView = view;
  node._kbJump = jump;
  node._kbStat = stat;
  node._kbBar = bar;
  ...
  node.addWidget("button", "🗑 Clear", null, () => {
    if (!confirm("Clear this chat?")) return;   // personas are untouched — only the history goes
    setHistory(node, []);
    ...
    nodeType.prototype.onExecuted = function (message) {
      ...
      if (p.reply != null) {
        ...
        if (mode === "fold" && p.fold) {
          ...
          if (user) h.push(tag({ role: "user", content: user }));   // APPEND — ↻ relies on this
