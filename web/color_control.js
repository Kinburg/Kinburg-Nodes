// Shared palette + native-picker control, used by Color Caption and Color Picker.

// Keep in sync with the PALETTE in the Python color nodes.
...
// Build a palette + native-picker control bound to a string widget. Returns its DOM root;
// `node.setDirtyCanvas` is called on every change. Sets `widget._knReflect()` so callers can
// re-sync the swatches/picker after a workflow load restores the widget value.
export function buildColorControl(node, widget, title, fallback) {
